...
print('Step1: Setting GCode files')

inputFileName = input("\tinput the GCode file name:")
outPutFileName = input("\tinput the output GCode file name:")
...
print('Step2: Material compensation begin...')
    ...
    if layer_count >= 1 and G91_count == 0 and ';' not in line and 'M' not in line:
        ...
        if 'G1' in line:
            if 'X' not in line:  # 这部分是回抽，不需要挤出补偿，比如G1 F6000 E27.06261
                ...
                # 提取"E"后面的数值部分并去除空格和换行符,并进行类型转换
                e_value = line[e_index + 1:].strip()
                e_value = float(e_value)
                e_value_modified = round(
                    previous_e_value_modified + e_value - previous_e_value, 5)
                # 重构line，写入文件
                modified_line = line[:e_index + 1] + \
                    str(e_value_modified) + "\n"  # 构建修改后的行
                ...
                # 提取"E"后面的数值部分并去除空格和换行符,并进行类型转换
                e_value = float(line[e_index + 1:].strip())

                # arc_length为基于基台的弧长，actual_arc_length 为实际的弧长
                arc_length = math.sqrt(
                    (x_value - previous_x_value) ** 2 + (y_value - previous_y_value) ** 2)

                actual_arc_length = math.sqrt(
                    # 因为Y坐标不存在长度问题
                    (x_value_actual - previous_x_value_actual) ** 2 + (y_value - previous_y_value) ** 2)

                # K为比例系数
                k = actual_arc_length / arc_length
                # 挤出量更改
                # k * e_value 按照比例关系进行补偿
                e_value_modified = round(
                    previous_e_value_modified + k * (e_value - previous_e_value), 5)
...
file_in.close()
file_out.close()
print('Step2: Material compensation finish!')
